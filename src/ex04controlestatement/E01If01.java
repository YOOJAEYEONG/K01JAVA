...
//		경우에따라 무조건 실행되는 if문도 필요함
		if(true) {
			System.out.println("무조건 실행되는 if문입니다.");
		}
		
//		if(조건식);=> 조건식에 상관없이 다음문장이 실행되는 형태로
...
//		조건검사 후 실행할 문장이 하나인 경우에는 {}중괄호를 생략할수 있다.
//		단, 실행문이 2개 이상인 경우에는 한문장만 실행되므로 주의해야한다.
		...
		System.in.read()
			...
			해당함수는 IOException을 기본적으로 가지고 있어 이를
			무시하기 위해 main()함수에서 throws 처리를 해준다.
		 */
...
//		[시나리오] 사용자가 입력한 문자가 숫자인지 판단하는 프로그램을 if문을 이용해 구현하시오.
		...
		※ 숫자의 구간을 정할때는 반드시 논리 AND를 사용해야 한다.
		 */
