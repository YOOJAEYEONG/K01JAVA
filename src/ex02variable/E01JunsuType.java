...
public class E01JunsuType {
	...
	public static void main(String[] args) {
		...
		byte byte1 = 50;
		byte byte2 = 55;
		int result2 = byte1 + byte2; //byte + byte = int형으로 자동형변환하여 연산됨.
		
		System.out.println("byte1 + byte2 = " + result2);
		System.out.printf("byte1 + byte2 = %d " , result2);
		...
		//Ctrl + Alt + 화살표위/아래 : 한줄 복사
		//long long3 = 2200000000; [에러발생]
		long long3 = 2200000000L; 
		long long4 = 2200000000l; /*
			long타입의 표현범위는 22억 이상인데 에러가 나는 이유는
			자바컴파일러(cpu)는 정수형상수를 기본적으로 int형으로 간주하기 때문에
			위 정수를 메모리에 올리는 순간 에러가 발생한다.
			이때는 접미사(L or l)를 붙여서 int형 자료가 아닌 long형 자료라는 것을
			알려줘야 에러가 발생하지 않는다.
			이와같이 대입을 위해 입력되는 숫자도 자료형을 기반으로 메모리에 저장되고,
			이를 가르켜 '리터럴(literal)' 이라고 한다.
			*/
		System.out.println("long3 = " + long3); //2200000000
		}
