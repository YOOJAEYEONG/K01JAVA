...
class Car{
	...
	int carYear;
	Human owner; /*
		객체변수 : Human 클래스를 기반으로 하는 멤버변수로 초기화를
		위해 이름, 나이, 에너지 세가지의 값이 필요하다.
		클래스를 구성할때 또 다른 객체를 멤버변수로 선언할수있다.
		ex)겜에서 케릭터가 무기(또다른객체)를 얻었을 경우를 생각.
	*/
...
//	객체의 초기화를 담당하는 메소드로 아래는 항상 같은 값으로만 초기화
//	된다는 한계를 가지고 있다. 
	void initialize() {
		carModel = "람보르기니";
		carYear = 2017;
...
//	초기화를 진행하는 역할은 동일하나, 매개변수를 통해 다양한 형태의 객체를 초기화 할수있다.
...
public class E03CarMain {
	...
	public static void main(String[] args) {
		...
		//car1.drive(); 
		/*	▲에러발생(자동차 소유주를 표현하는
			Human 객체가 초기화 되지 않은 상태에서
			출력을 시도하여 에러가 발생된다.
		*/
		System.out.println("[초기화메소드 호출후");
		car1.initialize();// 고정된 값으로 초기화 된다.
		car1.drive();
		car1.showCarInfo();
		
		//자동차2 객체 생성 : 객체생성후 멤버변수를 직접 각각 접근하여 초기화함.
		Car car2 = new Car();
		...
		//세번째 자동차 객체생성 : 매개변수가 있는 initialize(매개변수들)이용하여 초기화함.
