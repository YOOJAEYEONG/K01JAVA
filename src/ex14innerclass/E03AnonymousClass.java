...
-부모클래스의 메소드를 오버라이딩 하는것이 목적이다.(상속의 개념이 포함되어져있다)
-클래스를 정의하는 것이 메소드를 정의 하는 것과 비슷하다.
...
형식]
	...
	***부모클래스를 상속받아 이름없는 자식클래스를 생성하는 개념으로 이해하자.
...
//		Student라는 이름을 가진 자식클래스를 통해 객체를 생성하고 부모클래스의
//		참조변수로 참조함.
//		참조변수가 부모타입이므로 자식객체에 접근하는 것은 불가능.
//		반드시 다운캐스팅 후 접근해야한다.
		Person person = new Student("문사람", "2016");
		person.printInfo();
//		person.stNumber = "2015"//접근불가
...
//		하위익명클래스에서 확장한 변수이므로 익명객체를 통해서는 접근불가
//		anonyPerson.age =  100; // 접근불가
//		((Student)anonyStudent).age = 100;//다운캐스팅불가
		
		
		/*
		  
	 	다운캐스팅은 "(자식클래스명) 부모타입의 참조변수" 형태로 형변환하여
	 	하위클래스의 멤버에 접근하기위한 수단이다.
	 	그러나 익명클래스는 클래스명이 없기 때문에 위와 같이 다운캐스팅이 불가능하다.
	 	즉, 익명클래스에서는 새롭게 정의한 멤버변수를 접근할 수 없다.
	 	따라서 익명클래스는 오버라이딩의 목적으로만 사용해야한다.
		Person anonyTest = new Student("다운캐스팅", "안되요") {
			String jumin = "001024";
		};
		
		((Student)anonyTest).jumin = "18115";
		*/
