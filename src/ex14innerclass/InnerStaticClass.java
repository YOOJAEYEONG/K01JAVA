...
//	인스턴스형 멤버메소드
	void outerMemberMethod() {
		...
		innerStaticClass.innerMemberVar = 100;//내부 클래스의 멤버변수
		InnerStaticClass.innerStaticVar = 101;//내부 클래스의 정적변수
...
//	외부클래스의 정적멤버&메소드
	...
	static void outerStaticMethod() {
		/*
		외부클래스의 정적 멤버메소드에서는 내부클래스의 정적멤버만
...
//		인스턴스형 멤버
		...
		void innerMemberMethod() {
			System.out.println("s내부클래스의 멤버메소드 호출");
		}
//		정적멤버
...
//		**외부클래스가 아니라 다른 클래스에서 접근할 때의 방법은 아래와 같다.
...
//		내부정적 클래스의 인스턴스멤버에 접근할때는 객체를 생성한 후 참조변수를
//		통해서 접근해야한다.
