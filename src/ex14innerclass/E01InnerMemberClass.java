...
class OuterMember{
	...
	int outerInstanceVar;//일반적인 인스턴스형 멤버변수
	...
	void outerInstanceMethod() {
		...
		System.out.println("내부의 ㅓsameVar " + inner.sameVar);
	}
...
//		외부클래스는 내부클래스보다 넓은 지역이므로 내부클래스에서는 외부의 모든
//		멤버를 사용할 수 있다.
...
//		내부클래스에서는 static멤버를 선언할 수 없다. 단,,,,
//		상수(final)인 경우에는 허용 된다.
...
//		외부클래스가 아닌 다른클래스에서는 내부멤버클래스가 보이지 않으므로
//		직접 인스턴스를 생성할 수 없다.
//		InnerMember innerClass = new InnerMember(); //에러발생 
		
//		외부클래스의 인스턴스를 먼저 생성한후 내부클래스의 인스턴스를 생성해야한다.
