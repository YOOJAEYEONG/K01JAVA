 ...
 * 	class명 앞에는 public 과 default만 붙일 수 있다.
 */
public class E01AccessModifierMain
{
	...
	public static void main(String[] args)
	{
		...
		/*
		 * private로 선언된 멤버는 동일한 패키지 안에 있더라도 접근이 불가능하다.
		 * 해당 멤버는 동일 클래스 내에서만 접근을 허용한다.
		 
		 * private로 선언된 멤버를 외부클래스에서 접근할때는 주로 public으로 선언된
		 * 멤버 메소드를 통해 간접적으로 접근한다.
		 */
		...
		new DefaultClass1().myFunc();//객체 생성과 동시에 멤버 메소드 호출(12번줄 비교참고)
//		▲동일한 패키지에 있는경우 접근허용 확인
		///////////////////////////////////////////////////
//		▼다른 패키지에 있는경우의 접근허용 확인
		System.out.println("E01AccessModifier2 객체 생성 및 접근");
		
		/*
		 * 다른 패키지에 선언된 클래스이므로 객체 생성을 위해서는 import가 필요하다.
		 * 패키지가 다른경우 private, default 멤버의 접근을 허용하지 않는다.
		 */
		E01AccessModifier2 two = new E01AccessModifier2();
		
		//System.out.println("two.privateVar="+two.privateVar);//에러
		//System.out.println("two.defaultVar="+two.defaultVar);//에러
