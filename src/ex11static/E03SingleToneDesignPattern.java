 ...
 * 		2. 외부클래스에서 생성자를 접근할수 없으므로 new연산자를 통해 객체 생성은
 * 			불가능해진다.
 * 		3. 클래스 내부에 정적메소드로 해당클래스의 객체를 반환하도록 정의한다.
...
//	JVM(자바가상머신)에 의해 프로그램이 시작될때 정적변수는 메소드영역에 미리
//	생성되어 사용할 준비를 마치게 된다.
	private static SingleTone single = new SingleTone();
	
	
	
//	생성자를 private로 선언하면 클래스 외부에서는 접근할 수 없으므로
//	new연산자를 통해 객체생성은 불가능해지게 되는 원리.
	private SingleTone() {	}
...
public class E03SingleToneDesignPattern
{
	...
	{
//		일반적인 방식의 객체생성이므로 객체를 생성할때마다 새로운 주소를
//		할당받은 인스턴스가 생성된다
...
//		생성자를 접근할 수 없는 대신 getInstavce() 메소드를 통해서만
//		객체의 참조변수를 반환 받을 수 있다.
...
//		두 객체의 주소값은 동일하다
		...
		System.out.println(String.format("st2의 sharVar:%s", st2.shareVar));
		System.out.println(String.format("st3의 sharVar:%s", st3.shareVar));
