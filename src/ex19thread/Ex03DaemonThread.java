...
/*
쓰레드로 구현하지 않은 클래스
	: 하나의 클래스의 실행문이 끝나야 다음 클래스의 실행을 진행할 수 있다.
	즉 쓰레드처럼 동시에 작업을 진행 할 수 없다.
 */
class NotThread{
	String title;
...
독립쓰레드(Non Daemon Thread)
	...
	메인 쓰레드가 종료되어도 계속 실행되며, Dead상태가 되어야 실행이 종료되는
	쓰레드를 말한다.
	해당 쓰레드를 실행하게 되면 main메소드가 종료 된 이후에도 실행되는 것을 볼수있다.
 */
class YesThread extends Thread{
	...
	public YesThread() {}
	/*
	Thread클래스의 생성자를 호출할때 전달하는 매개변수를 통해 해당 쓰레드의 
	이름을 지정할 수 있다. 지정된 이름은 getName()메소드를 통해 반환할 수 있다.
	 */
...
종속 쓰레드 (Daemon Thread)
	...
	프로그램이 종료될때 자동으로 종료되므로 주로 무한 루프로 구성한다.
	setDaemon(true)메소드로 데몬쓰레드를 설정할 수 있다.
 */
...
public class Ex03DaemonThread {
	...
	public static void main(String[] args) {
		 ...
		 */
		YesThread yt1 = new YesThread("1st 쓰레드");//생성자를통해 이름을 설정하는 방법1
		yt1.setName("첫번째  쓰레드");//setName()을통해서 이름을 설정하는 방법2
