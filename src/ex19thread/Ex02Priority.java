...
class MessageSendingThread extends Thread{
	...
	public void run() {
		for(int i=1; i<=1000 ; i++) {
			...
			메소드를 사용하게 되면 실행중 블럭상태로 전환되어 우선순위가 낮은
			쓰레드도 가끔 실행될 수 있는 상황이 생기게 된다.
			 */
			try {
				sleep(3);//0.003초 동안 블럭상태로 만듬
...
public class Ex02Priority {
	...
	public static void main(String[] args) {
		...
		/*
		1단계
			: 우선순위를 별도로 지정하지 않은 상태로 쓰레드를 생성하면
			동일한 우선순위 5를 부여받게 된다.
...
//				new MessageSendingThread("세번째");
		...
		2단계
			...
			***우선순위가 높다고 무조건 먼저 실행 되지는 않는다.
			CPU의 유휴상태가 생기면 우선순위가 낮은 쓰레드도 실행될 수 있다.
		 */
		MessageSendingThread tr1 = 
		...
		/*
		3단계
			: static 상수 대신에 정수를 사용해도 된다. 단 가독성이 떨어짐
		 */
//		MessageSendingThread tr1 = 
