...
큰 차이점이라면 매개변수의 유무, 리턴값의 유무이다.

Consumer<T>	: 매개변수 있고, 리턴값 없음 (매개변수를 소비만 하는 형태로 기억하자)
Supplier<T>	: 매개변수 없고 , 리턴값 있음
Function<T>	: 매개변수,리턴값 둘다 있음
Predicate<T>: 매개변수 있고, boolean리턴 
*/
public class Ex06Define1Predicate {
	...
	public static void main(String[] args) {
		...
		: 전달된 인자를 대상으로 true, false를 판단할 때 사용하는 
			추상메소드를 정의하고 있다.
		 */
		...
		/*
		Arrays.asList() : 일반적인 배열을 List컬렉션으로 변형해주는 메소드
		배열과 컬렉션의 중간단계라고 함
		 */
		...
		//a는 인자값
		double maleAvg = 
				avg(a -> a.getGender().equals("남"), list1);
	...
	/*
	매개변수로 성별을 판한하는 람다식과 
	List컬렉션을 전달받아 평균값을 반환하는 메소드
	 */
	public static double avg(Predicate<Human> ph, List<Human> li) {
		int pCount = 0;//전달된 객체의 성별에 따라 인원수 카운트
		int tCount = 0;//점수를 누적해서 더해 총점을 저장하기 위한 변수
		for(Human h : li) {//li 컬랙션의 크기 만큼 반복
			/*
			첫번째 호출에서는 남자일때 true를 반환한다.
			즉, 남자의 점수가 카운트됨
			두번째 호출은 여자
			 */
			if(ph.test(h)==true) {
	...
	public static int sum(Predicate<Integer> isBoolean, List<Integer> list) {
		int sum = 0;
		for(int a : list) {
			if(isBoolean.test(a)) {
				sum += a;
