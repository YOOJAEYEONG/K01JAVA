...
//	integer : 기본자료형 데이터를 객체로 포장해주는 역할 
...
//	class앞에 abstract를 지우면 에러가 발생한다.
...
부모가 가진 추상메소드를 그대로 상속받게 되므로 에러가 나지 않으려면
	1. 해당클래스도 abstract로 선언하거나
	2. 부모의 abstract메소드를 오버라이딩 처리해야한다.
 */
class absClassChild2 extends absClass2{
	
	@Override
	void absMethod(int number) {
		System.out.println("부모클래스에서 오버라이딩한 메도스로 없다면 에러발생함");
	}
