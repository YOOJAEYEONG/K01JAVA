...
클래스는 반드시 추상메소드를 오버라이딩 해야한다.
-클래스의 상속관계에서의 설계도 역할을 한다.
-객체생성은 할수 없으나 그외 참조변수, 상속등의 모든 기능을 가지고 있다.

추상 메소드
-오버라이딩의 목적으로 만들어진다.
-함수의 실행부가 없기 때문에{}를 쓰지 않고 삼수의 원형에 ;을 붙여준다.
 */

//프로젝트 전체에서 기준이 될수있는 추상클래스를 정의
...
//	전달받은 인자를 통해 PersonalInfoDTO 객체를 생성후 객체배열에 저장한다.
	@Override
...
//	주민번호를 인자로 전달받아 객체배열에 저장된 정보를 검색한 후 일치하는 경우에만 이름을 반환한다.
	...
	public String searchPersonalInfo(String juminNum) {
		...
		return null;//검색결과가 없는 경우 null을 반환
	}
}
public class E02AbstractToInterface1 {
	...
	public static void main(String[] args) {
		...
		//정보저장
		storage.addPersonalInfo("901234-22222222", "김태희");
		storage.addPersonalInfo("901234-11111111", "정지훈");
		
		//검색
		System.out.println(storage.searchPersonalInfo("901234-22222222"));
		System.out.println(storage.searchPersonalInfo("901234-11111111"));
		System.out.println(storage.searchPersonalInfo("901256-11154111"));
	}
}
