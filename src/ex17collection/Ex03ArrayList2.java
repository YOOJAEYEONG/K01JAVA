...
public class Ex03ArrayList2 {
	...
	public static void main(String[] args) {
		...
		/*
		<2차원 컬렉션>
		배열처럼 저장되어있는 컬렉션에서 객체 안에 데이터를 3개씩 가지고있다. 
		즉, 2차원 배열의 형태를 가지고있다는 말이다.
		제네릭에서 사용하는 타입매개변수는 항상 객체이다 int 같은 타입은 사용불가
		List계열의 컬렉션 생성시 타입매개변수를 Student형으로 선언함.
		 */
		LinkedList<Student> list2 = new LinkedList<Student>();
		//ArrayList<Student> list2 = new ArrayList<Student>();
		//위 두줄을 하나씩 주석을 풀어 실행해보면 문제가 없다
...
//			student.showInfo();
			it2.next().showInfo();
			//다 체인 기법 : .으로 계속 연결하는것
			//위 두줄로 써도 되고 아래처럼 다체인기법으로 사용해도 된다. 동일한문장임
			//next()를 통해 객체를 가져온후 showInfo() 로 객체의 내용을 출력
		}
