...
-Set처럼 중복저장은 허용하지 않으나 정렬된 순서로 저장된다.
...
class MyString implements Comparable<MyString>{
	...
	문자열의 길이등 다양하게 설정 가능하다. 아래코드는 오름차순(작은->큰)
	정렬일때 사용한다. 1과 -1의 위치가 바뀌면 내림차순이 된다.
	 */
	
	//compareTo(): 알파벳 사전순으로 비교
	@Override
	public int compareTo(MyString paramStr) {
		//정렬 순서는 문자열의 길이를 기준으로 한다.
		if(getLength() > paramStr.getLength())
			return 1;//1, -1을 바꾸면 오름차순 내림자순을 바꿀수있다
