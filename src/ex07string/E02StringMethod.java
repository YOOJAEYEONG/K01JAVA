...
//			index는 반드시 0부터 시작한다.
		...
		System.out.println("str2의 첫번째 문자:"+ str1.charAt(0));
...
//			같으면 0을 반환
		...
		String str3 = "A";
		String str4 = "B";
		System.out.println(str3.compareTo(str4));//-1 이걸어디에 활용할지 궁굼
		System.out.println(str4.compareTo(str3));//1
		System.out.println(("ABC".compareTo("ABC")==0) ?
				"문자열이 같다." : "문자열이 다르다");
		
//		5] concat() : 두개의 문자열을 연결할때 사용한다. + 기호와 동일한 역할임.
		System.out.println("JAVA".concat(" world").concat(" go"));
		System.out.println("JAVA"+" world"+" go");
		
//		6] contains() : 문자열에 특정 문자열이 포함되어있는지 판단하여
//			포함되어있으면 true, 아니면 false를 반환한다.
...
//		7] endsWith()
//			: 문자열이 특정문자열로 끝나면 true, 아니면 false를 반환한다.
//		8] startsWith()
//			: 문자열이 특정 문자열로 시작하면 true, 아니면 false를 반환한다.
		System.out.println("www.daum.net".endsWith("net"));
		System.out.println("never.com".startsWith("http"));
		
//		9]format() 
//			: 출력형식을 지정하여 문자열로 반환하고 싶을때 사용.
//			Printf()와 사용법은 동일하고 주로 웹어플리케이션(JSP)를 제작할때 사용된다.
...
//		시나리오] 주민번호를  indexOf를 사용하여 성별을 구분하는 프로그램을 구하시오.
...
//			특정문자열을 찾아 인덱스를 반환한다.
		...
		System.out.println("indexOf:" 
				+ str1.indexOf("a"));//12(앞에있는a의인덱스) 
		System.out.println("lastIndexOf:" 
				+ str1.lastIndexOf("a"));//14(뒤에있는a의 인덱스)
		
		
//		12]replace
//			특정문자열을 찾아서 지정된 문자열로 변경한다.
//			특정문자열이 없으면 변경되지 않는다 (에러X)
		System.out.println("J를 G로 변경하기");
		System.out.println(str1.replace("J",  "G"));//Welcome to java(변화없음)
		System.out.println("java를 KOSMO로 변경하기");
		System.out.println(str1.replace("java", "KOSMO"));//Welcome to KOSMO
		
//		13]	split()
//			: 문자열을 특정 구분자(토큰)으로 분리해서 String타입의 배열로 반환한다. 
//			해당 구분자가 없는 경우에는 배열의 크기가 1인 String형 배열로 반환된다.
...
//			즉 문자열을 자를때 사용하는 메소드.
//			사용법1 : substring(index) -> 인덱스부터 문자열의 끝까지를 반환
//			사용법2 : substring(index1, index2) -> 인덱스에 포함되는 부분만 반환
		System.out.println(str1.substring(3));//come to java
		System.out.println(str1.substring(3,7));//come
		...
		int beginIndex = filename.lastIndexOf(".")+1;//15
