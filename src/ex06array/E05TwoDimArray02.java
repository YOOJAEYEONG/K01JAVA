...
//		세로크기3, 가로크기4인 2차원 배열 선언. 3행4열
		int[][] arr2Dim = new int[3][4];
		
//		가로,세로 크기만큼 반복하면서 초기화를 진행
		for(int i=0 ; i<=2 ; i++)
			for(int j=0; j<=3 ; j++)
				arr2Dim[i][j] = i + j;
...
//		먼저 세로크기만큼 반복후 가로크기만큼 반복
		for(int i=0 ; i < twoArray.length ; i++)//세로크기:배열명을 그대로 이용
			for(int j=0 ; j<twoArray[i].length ; j++)//가로크기:배열명[인덱스]를 이용해 각행의 크기를 가져욤
...
//		배열의 각 요소값을 전달한 매개변수만큼 증가.여기서는 10만큼증가
...
//	배열을 출력하는 부분이 증복되기 때문에 이런경우 메소드를 생성하여
//	호출해주는 것이 좋다.
	...
	static void twoDimPlus(int[][] arr2, int plusNum) {
		for(int i=0 ; i < arr2.length ; i++) {
			for(int j=0 ; j<arr2[i].length ; j++)
				arr2[i][j] += plusNum;
//				매개변수로 전달된 plusNum 만큼 각 인덱스값을 증가시킴
