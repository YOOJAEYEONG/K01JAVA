...
public class E01WrapperClass2 {
	public static void main(String[] args) {
		...
		System.out.println("10+strNumber를 숫자로 변경="
				+(10+Integer.parseInt(strNumber)));//반환타입(int)
		System.out.println("10+strNumber를 숫자로 변경="
				+(10+Integer.valueOf(strNumber)));//동일한 기능 (반환타입:Integer)
...
//		System.out.println(Integer.parseInt(floatString));//에러발생
		...
		/*
		codePointAt() : 문자열에서 index에 해당하는 한 문자의 아스키코드 값을 반환.
		 */
		...
		/*
		3]	isDigit() : 인자로 전달된 문자가 숫자인지 판단하여 boolean 타입으로 반환.
		 */
		System.out.println("isDigit()를 통한 숫자판단");
		System.out.println(Character.isDigit('A')?"숫자임":"숫자아님");//숫자아님
		System.out.println(Character.isDigit(50)?"숫자임":"숫자아님");//숫자임
		System.out.println(Character.isDigit('7')?"숫자임":"숫자아님");//숫자임

		/*
		4]	isLetter() : 문자여부를 판단하는 메소드로 특수기호나 숫자형은 false를 반환
		 */
		...
		/*
		6]	대소문자를 판단하는 메소드 
			: 영문자만 적용되며 알파벳이 아닌경우 false를 반환한다.
		...
		System.out.println(Character.isLowerCase('가'));//알파벳이 아니므로 그냥 false
		
		
		/*
		시나리오] 주어진 문자열안에 몇개의 공백문자(스페이스)가 있는지 판단하는 프로그램을 구하시오.
			참고)toCharArray() : 문자열을 char형 배열로 반환해주는 String클래스에 정의된 메소드
		 */
		String whiteString = "H E	LLO";//공백하나, 탭하나를 입력해봄
		int whiteCount = 0;
