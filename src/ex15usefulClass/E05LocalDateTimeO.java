...
public class E05LocalDateTimeO {
	...
	public static void main(String[] args) {
		...
		System.out.println("2020년 3월 기분, 첫번째 일요일: "+ first12Sunday);
		
		LocalDate xmas = LocalDate.of(LocalDate.now().getYear(),12,25);
		Period left = Period.between(nowDate, xmas);
		System.out.println("Xmax까지 앞으로 "+ left.getMonths()+"개월"+left.getDays()+"일");
		
		Instant end = Instant.now();
		Duration duration = Duration.between(start, end);
		System.out.println("클래스 실행 시간 확인 (두시간의 차) : "+ 
				duration.toMillis());
