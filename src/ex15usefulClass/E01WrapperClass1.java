...
public class E01WrapperClass1 {
	...
	public static void main(String[] args) {
		...
		System.out.println("결과값을 byte값으로 변경"+resultObj.byteValue());
		
		System.out.println("결과값을 이진수로 변경"+Integer.toBinaryString(result2));
		
		//JDK1.5이후 버전에서의 코딩 형태
		...
		/*
		자바의 모든 클래스는 Object클래스를 상속받는댜.
		그러므로 아래 메소드를 사용할 수 있다.
		
		toString(): 인스턴스 변수를 출력시 참조값이 아니라 실제 저장된 값을
					문자열 형태로 반환해주는 메소드
		hashCode():	객체를 구별하기위해 참조값을 고유한 정수값으로 반환해주는 메소드
		 */
		Integer numObj = new Integer(99);
		
		System.out.println("numObj="+ numObj);//내부적으로 toString()의 도움을 받는다.
		System.out.println("numObj.toString()="+numObj.toString());
		System.out.println("numObj.hashCode()="+numObj.hashCode());
		System.out.println(numObj+1);//99+1
		System.out.println(numObj.toString()+1);//"99"+1
	}
}
