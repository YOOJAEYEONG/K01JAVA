...
/*
 * 오버라이딩(Overriding)
 * 	: 클래스가 상속관계에 있을때 하위클래스에서 상위클래스의 멤버메소드와 완전히
 * 동일한 모양으로 재정의하는것을 말한다.
 * 오버로딩(Overloading)과 다른점은 매개변수의 갯수, 타입, 반환형이 완전히 
 * 똑같은 형태로 정의 하는 것이다.
 * 하위클래스에서 오버라이딩한 메소드는 상위클래스의 메소드를 가리게 되므로 항상
 * 최하위 클래스에서 오버라이딩한 메소드가 호출되게 한다.
 */
...
class BaseSpeaker extends Speaker{
	...
	/*
	 * @Override : '어노테이션'이라 부르고 오버라이딩된 메소드에 붙어 
	 * 				컴파일러에게 알려주는 역할을 한다.
	 */
	@Override
	public void showState() {
...
public class E06Overriding {
	...
	public static void main(String[] args) {
		...
		*/
		System.out.println();
		/*
		 * 부모 클래스의 참조변수가 자식클래스의 객체를 참조할 수 있다.
		 * 이경우 접근 범위는 부모클래스로 제한된다.
...
//		speaker.setBase(40);//에러발생
		speaker.showState();//부모클래스가 아닌 자식클래스의 메소드가 호출하게 된다.
