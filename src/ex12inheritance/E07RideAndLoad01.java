 ...
 * 
 * 오버로딩 : 함수명은 같으나 매개변수의 갯수, 타입, 숫자가 다른경우를
 * 말한다. 즉, 서로 다른 함수이므로 상속받은 하위클래에는 오버로딩된 
 * 모든 메소드가 포함된다.
 * 
 * 오버라이딩 : 함수명, 매개변수, 반환형까지 모두 동일한 함수를 
 * 	상속관계에 있는 하위클래스에서 재정의한것을 말한다.
 	이경우 하위 클래스에서 정의한 메소드가 상위 클래스의 메소드를 가리게 되므로
 	항상 최하위 클래스의 메소드가 호출되게 된다.
...
//		해당 객체를 B타입으로 간주하고 참조한다.
		ref2.loadMethod();
		ref2.loadMethod(1); //-> B 클래스 영역까지 접근 가능
//		ref2.loadMethod(1.1);//접근 불가
		
//		해당 객체를 A타입으로 간주하고 참조한다.
		ref1.loadMethod(); // -> A클래스 영역까지 접근 가능
...
//		refId1은 A타입의 참조변수이므로 B타입 혹은 C타입의 참조변수로
//		참조할 수 없다. 자식타입의 참조변수로 부모 객체를 참조하는 것은 불가능하다.
		A refId1 = new C();
//		B refId2 = refId1; //자식으로 부모를 참조하는 형태
//		C refId3 = refId1;//자식으로 부모를 참조하는 형태
