...
public class E07RideAndLoad02 {
	...
	public static void main(String[] args) {
		...
		child.study(); //자식에서 확장
		child.walk(); // 오버로딩, 부모쪽의 walk()호출
		child.walk(25); // 오버로딩, 자식쪽의 walk(int)호출
...
//		정적메소드 - 클래스명으로 호출함
		...
		/*
		 * 오버라이딩 처리된 메소드이므로 참조변수에 상관없이
		 * 무조건 자식 메소드가 호출됨. 참조변수의 영향을 받지 않는다.
		 */
		parent.excecise();
		parent.sleep();
		parent.printParent();
		
//		parent.study();//에러발생/ 자식에서 확장된 메소드 - 접근불가
		parent.walk();
//		parent.walk(20);//에러발생/  자식에서 확장된 메소드 - 접근불가
